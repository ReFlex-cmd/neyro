...
# Параметры для экспериментирования
IMG_SIZE = 100  # Размер изображения
BATCH_SIZE = 64
LEARNING_RATE = 0.001

# Изменяемые параметры для экспериментов
LAYERS_TO_TEST = [2, 3, 4, 5]  # Диапазон числа слоев
EPOCHS_TO_TEST = [7, 8, 9, 10]  # Диапазон эпох
ACTIVATION_FUNCTION = 'leaky_relu'  # Фиксированная функция активации
...
# Гибкая нейросеть с изменяемыми слоями и фиксированной активацией
...
def experiment_parameters():
    ...
    for num_layers in LAYERS_TO_TEST:
        for num_epochs in EPOCHS_TO_TEST:
            print(f"\n### Эксперимент: слоев={num_layers}, эпох={num_epochs} ###")

            # Создаем модель
            model = FlexibleFFNN(input_size, hidden_size, num_classes, num_layers)
            model.to(device)
            ...
            # Сохраняем модель
            torch.save(model.state_dict(), f'results/model_layers{num_layers}_epochs{num_epochs}.pth')
            ...
            plt.title(f'Потери (L={num_layers}, E={num_epochs})')
            plt.xlabel('Эпоха')
            plt.ylabel('Потери')
            plt.legend()
            ...
            plt.title(f'Точность (L={num_layers}, E={num_epochs})')
            plt.xlabel('Эпоха')
            plt.ylabel('Точность (%)')
            plt.legend()

            plt.tight_layout()
            plt.savefig(f'results/training_plot_L{num_layers}_E{num_epochs}.png')
            plt.close()
...
def visualize_experiment_results(results_df):
    ...
    # Точность
    plt.subplot(2, 2, 1)
    for num_epochs in results_df['num_epochs'].unique():
        df_subset = results_df[results_df['num_epochs'] == num_epochs]
        plt.plot(df_subset['num_layers'], df_subset['test_acc'], marker='o', label=f'{num_epochs} эпох')
    plt.title('Зависимость точности от числа слоев и эпох')
    ...
    for num_epochs in results_df['num_epochs'].unique():
        df_subset = results_df[results_df['num_epochs'] == num_epochs]
        plt.plot(df_subset['num_layers'], df_subset['f1'], marker='o', label=f'{num_epochs} эпох')
    plt.title('Зависимость F1-меры от числа слоев и эпох')
    plt.xlabel('Число скрытых слоев')
    plt.ylabel('F1-мера (%)')
    plt.legend()
    plt.grid(True)
    ...
    for num_epochs in results_df['num_epochs'].unique():
        df_subset = results_df[results_df['num_epochs'] == num_epochs]
        plt.plot(df_subset['num_layers'], df_subset['avg_epoch_time'], marker='o', label=f'{num_epochs} эпох')
    plt.title('Зависимость времени обучения от числа слоев и эпох')
    plt.xlabel('Число скрытых слоев')
    ...
    for num_epochs in results_df['num_epochs'].unique():
        df_subset = results_df[results_df['num_epochs'] == num_epochs]
        plt.plot(df_subset['num_layers'], df_subset['final_train_loss'], marker='o', label=f'{num_epochs} эпох')
    plt.title('Зависимость потерь от числа слоев и эпох')
    plt.xlabel('Число скрытых слоев')
...
def main():
    ...
    best_model_path = f"results/model_layers{int(best_config['num_layers'])}_epochs{int(best_config['num_epochs'])}.pth"
