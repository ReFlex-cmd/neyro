...
def experiment_parameters():
    ...
    for num_layers in layers_to_test:
        for activation in activations_to_test:
            ...
            # Сохраняем модель
            torch.save(model.state_dict(), f'results1/model_layers{num_layers}_{activation}.pth')
            ...
            plt.tight_layout()
            plt.savefig(f'results1/training_plot_L{num_layers}_A{activation}.png')
            plt.close()
    ...
    results_df.to_csv('results1/experiment_results.csv', index=False)
...
def visualize_experiment_results(results_df):
    ...
    plt.tight_layout()
    plt.savefig('results1/parameter_comparison.png')
    plt.show()
    ...
    plt.grid(axis='y')
    plt.tight_layout()
    plt.savefig('results1/activation_comparison.png')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('results1/accuracy_heatmap.png')
    plt.show()
...
def analyze_misclassifications(model, test_loader, class_names):
    ...
    plt.tight_layout()
    plt.savefig('results1/error_types.png')
    plt.show()
    ...
    if len(errors) > 0:
        ...
        for i, ax in enumerate(axes.flat):
            if i < min(9, len(errors)):
                ...
                ax.axis('off')
        plt.tight_layout()
        plt.savefig('results1/error_examples.png')
        plt.show()
...
def main():
    ...
    best_model_path = f"results1/model_layers{int(best_config['num_layers'])}_{best_config['activation']}.pth"
    ...
    print("\nВсе эксперименты завершены. Результаты сохранены в директории 'results1/'")
